...
export default class Drugs extends RequestHandlers{
    ...
    handle(req: any, res: any): void {
        ...
        this.getDrug(DRUG_ID).then((drug:{})=>res.json({drug})).catch((err:any)=>res.json({error:err}))
    }


    private getDrug(DRUG_ID:number){
        const query=`select DRUG_ID,DRUG_NAME,BRAND_NAME,MENUFACTURER,BRAND,DAR,PRICE,IMAGE_SRC from Drugs where DRUG_ID=?`
