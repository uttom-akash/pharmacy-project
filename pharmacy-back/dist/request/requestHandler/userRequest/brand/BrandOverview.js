    ...
    BrandOverview.prototype.handle = function (req, res) {
        ...
        this.getBrandsName().then(function (brandsList) {
            var drugs = brandsList.map(function (brand) {
                ...
                return Promise.all([brandedDrugs]).then(function (brandedDrugs) { return ({ BRAND: brand["BRAND"], DRUGS: brandedDrugs }); });
            });
            Promise.all(drugs).then(function (DRUGS) { return res.json(DRUGS.map(function (drug) { return ({ BRAND: drug['BRAND'], DRUGS: drug['DRUGS'][0] }); })); });
        });
    };
